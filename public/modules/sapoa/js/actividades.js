...
$.fn.eventos = function(){
	...
	$('.institucional').unbind('click');//borro evento click
	$('.institucional').on("click", function(e) { //asigno el evento change u otro
	...
	$('.estrategico').unbind('click');//borro evento click
	$('.estrategico').on("click", function(e) { //asigno el evento change u otro
	  
	   id_estrategico= e.target.value;

	   

	   $.fn.get_iniciativa(id_estrategico)
	});
...
let desc =  ( element.nombre.substr(0, 80).length == 80 ) ?  element.nombre.substr(0, 80) + '...' : element.nombre.substr(0, 80);
	 	...
	 	$.ajax({url: "/institucional/estrategico/"+ id_institucional+'/'+ id_nacion ,
			method: 'GET',}).then(function(result){
				console.log('resultado',result);
				$('#estrategico').html('<label for="">Objetivo estratégico</label></br>');
...
let desc =  ( element.nombre.substr(0, 80).length == 80 ) ?  element.nombre.substr(0, 80) + '...' : element.nombre.substr(0, 80);
 ...
 $.fn.get_iniciativa= function ($id_institucional) {

	 	$.ajax({url: "/iniciativa/estrategico/"+ id_estrategico ,
			method: 'GET',}).then(function(result){
				console.log('resultado',result);
...
let desc =  ( element.nombre.substr(0, 80).length == 80 ) ?  element.nombre.substr(0, 80) + '...' : element.nombre.substr(0, 80);
    			...
    			`<p>
    				...
    				class="iniciativa" 
    				name="iniciativa" 
    				value="${element.id_iniciativa}" 
    				id="obj-${element.id_iniciativa }"
    				/> 
    				${desc} 
    			</p>`);
